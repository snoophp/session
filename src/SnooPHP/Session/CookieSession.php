...
class CookieSession extends Session
{
	...
	/**
	 * Check if user is authenticated and redirects it otherwise
	 * 
	 * @param string $redirectUrl url unauthenticated user are redirected to
	 */
	public static function check($redirectUrl = "/login")
	{
		if (!static::id())
		{
			$requestUrl = Request::current()->url();
			new Redirect($redirectUrl."?redirect=".$requestUrl, 302);
		}
	}
	 ...
	 */
	protected static function createSession($user, $token, $stayLogged = false)
	{
		/* @todo use field map */

		$sessionMaxLength	= function_exists("sessionConfig") ? sessionConfig("session_max_length") : self::SESSION_MAX_LENGTH;
		$sessionDefLength	= function_exists("sessionConfig") ? sessionConfig("session_def_length") : self::SESSION_DEF_LENGTH;
	 ...
	 * Refresh session
	 */
	protected static function refreshSession()
	{
